    ...
    const cityWeather = async() => {
        try{
            ...
            const backGroundPics = await axios.get(
                `https://api.unsplash.com/search/photos?page=1&per_page=10&query=${newCity !== "" ? newCity : currentPlace.data.city}&client_id=${process.env.REACT_APP_BG_KEY}`
            )
                setCityPictures(backGroundPics.data.results[Math.floor(Math.random()*10)].urls.regular)
            
            const weatherInfo = await axios.get(
                `http://api.openweathermap.org/data/2.5/weather?q=${newCity !== "" ? newCity : currentPlace.data.city}&units=metric&appid=${process.env.REACT_APP_API_KEY}`
            )
                setDataStorage({
            ...
            const futureWeather = await axios.get(
                `http://api.openweathermap.org/data/2.5/onecall?lat=${weatherInfo.data.coord.lat}&lon=${weatherInfo.data.coord.lon}&exclude=minutely&units=metric&appid=${process.env.REACT_APP_API_KEY}`
            )
                setFutureWeather({
                    // Hourly
